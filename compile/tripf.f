...
!-----------------------------------------------------------------------
      ...
      integer num_modes(nwalls)

      tamps = wallpar(1)
      tampt = wallpar(2)
      tdt   = wallpar(3)
      num_modes(1) = int(wallpar(4))
      num_modes(2) = int(wallpar(6))
c
...
c
         do k=1,nwalls 
           call rand_func(fzt2(1,k),znek(1,k),kpts(k),seed,num_modes(k))
            do z=1,kpts(k)
...
c
         do k=1,nwalls
           call rand_func(fzt4(1,k),znek(1,k),kpts(k),seed,num_modes(k))
            do z=1,kpts(k)
...
c    read TRIP FORCING parameters from forparam.i
      ...
      if (istep.eq.0.) then
         ...
         read(1011,*) radiusx_up
         if(nid.eq.0)  write(6,*) 'radius =  ', radiusx_up
         read(1011,*) radiusy_up
         if(nid.eq.0)  write(6,*) 'radius =  ', radiusy_up
         read(1011,*) radiusx_lo
         if(nid.eq.0)  write(6,*) 'radius =  ', radiusx_lo
         read(1011,*) radiusy_lo
         if(nid.eq.0)  write(6,*) 'radius =  ', radiusy_lo
         read(1011,*) tdt
